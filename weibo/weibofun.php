...
class SaeTOAuthV2 {
	 ...
	 * @ignore
	 */
	public $useragent = 'zisheng.org'; //-->public $useragent = 'Sae T OAuth2 v0.1';
	...
	function getAuthorizeURL( $url, $response_type = 'code', $state = NULL, $display = NULL ) {
		...
		return $this->authorizeURL() . "?" . http_build_query($params);  //-->http_build_query生成client_id=xxx&redirect_uri=xxx&
	}

        /****
         * 结束token，等同于退出登陆 参考 http://open.weibo.com/wiki/2/account/end_session
         *
         *
         *
         *
        ...
        function getLogoutURL($token){
            ...
            return "https://api.weibo.com/2/account/end_session"."?".http_build_query($params);
        }
	...
	function oAuthRequest($url, $method, $parameters, $multi = false) {
		...
		if (strrpos($url, 'http://') !== 0 && strrpos($url, 'https://') !== 0) {
			$url = "{$this->host}{$url}.{$this->format}";    //--> https://api.weibo.com/2/$url.json
	}
	...
	function http($url, $method, $postfields = NULL, $headers = array()) {
		...
		$response = curl_exec($ci); //-->返回执行结果
		$this->http_code = curl_getinfo($ci, CURLINFO_HTTP_CODE);
		$this->http_info = array_merge($this->http_info, curl_getinfo($ci)); //-->后一个数组内容加到前一个数组的后面
		$this->url = $url;
...
class SaeTClientV2
{
	...
	function public_timeline( $page = 1, $count = 50, $base_app = 0 )
	{
         ...
         *
         **/
        function friend_timeline( $page = 1, $count = 50, $since_id = 0, $max_id = 0, $base_app = 0, $feature = 0 )
	{
		$params = array();
		if ($since_id) {
		...
		return $this->oauth->get('statuses/bilateral_timeline', $params);
	}
	/**
	 ...
	 *  - medium:媒体总编
	 *  - stockplayer:炒股高手
	 *  已经改了  (DEFAULT,ENT,MUSIC,FASHION,LITERATURE,BUSINESS,SPORT,SPORTS,HEALTH,AUTO,HOUSE,TRIP,STOCK,FOOD,FATE,ART,TECH,CARTOON,GAMES,MEDIUM,MARKETER)
	 * @return array
	 */
